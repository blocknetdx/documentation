...
[Block DX](introduction.md) is a decentralized exchange dApp powered by the Blocknet Protocol. Unlike centralized exchanges, Block DX has no deposit, withdrawal, or maker fees other than network fees of the asset being traded.
...
Having no fee to place an order encourages market makers to add liquidity. This also makes it possible for a user to acquire the [BLOCK](../blockchain/introduction.md) needed to take orders.

---

## Taker Fee
When accepting an order on Block DX, a static fee of 0.015 [BLOCK](../blockchain/introduction.md) is charged at the time the order is taken. This fee is charged even if a trade is canceled or fails and is meant to discourage malicious behavior on the network. 
...
If the taker asset is BLOCK, there needs to be *at least* two [UTXOs](../resources/glossary.md#utxo) - one or more to cover the 0.015 BLOCK fee and one or more to cover the traded amount. 
...
## Fees Distribution
100% of trade fees are distributed to [Service Node](../service-nodes/introduction.md) operators for supporting the network.
